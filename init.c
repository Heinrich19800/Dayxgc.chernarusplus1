...
//<!---------------------------------  ADMIN  -------------------------------->
	...
	{
	bool IsAdmin = false;  
	string SteamId_AdminDZ = "76561198007673694"; // Steam id

	if (player.GetIdentity().GetPlainId() == SteamId_AdminDZ){IsAdmin = true;}
	if (IsAdmin){
...
//Admin Tool// ---------------------------------------------------------------------------------------------------------
			...
			case "slap": {
				...
				if(temp_player == NULL) {
					...
					temp_player.SetPosition(temp_player.GetPosition() + "1 3 0");
					SendMessageToPlayer(player, "You slapped " + temp_player.GetIdentity().GetName());
